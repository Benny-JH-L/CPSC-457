...
) 
{
    ...
    vector<int64_t> vec_remaining_proc_time(processes.size());    // index == process ID, and it contains the remaining time it has until it finishes
    ...
    while (true)
    {
        ...
        if (ready_queue.size() == 0 || ready_queue.empty())     // need to add proccesses to ready queue (rq)
        {
            ...
            {
                int proc_jq = job_queue.front();    // proccess to check
                // cout << "proc id <"<<proc_jq<<"> arrival: " << processes[proc_jq].arrival << endl;  // debug

                // if this proccess's arrival <= current time add it to the ready queue, as it `arrived` during another proccess' execution 
                if (processes[proc_jq].arrival <= current_time) 
                {
                    ready_queue.push(proc_jq);  // add to ready queue 
                    job_queue.pop();            // remove from job queue
            ...
            current_time = max(current_time, processes[id].arrival);    // update the current time
        ...
        while (!job_queue.empty())
        {
            ...
            // add the proccess from job queue to ready queue
            if (processes[proc_jq].arrival < current_time)
            {
        ...
        if (((ready_queue.size() == 1 && seq.size() < max_seq_len)) && num_proc_started >= ready_queue.size())
        {
            cout << "hey :)\n";
            ...
            queue<int> copy_rq = ready_queue;
            while (!copy_rq.empty() && copy_rq.size() > 0){
                int id = copy_rq.front();
                copy_rq.pop();
                int64_t cur = vec_remaining_proc_time[id];
            ...
            if (!job_queue.empty()){
                n2_val = (processes[job_queue.front()].arrival-current_time)/(rounds * quantum);
            }

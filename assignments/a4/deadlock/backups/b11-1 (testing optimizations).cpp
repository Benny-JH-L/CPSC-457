...
public:
    ...
    // return's true if a cycle was detected, false otherwise. If a cycle was detected, `vec_proc_in_deadlock` will contain node indices that make up the cycle
    ...
    {    
        ...
        if (removed_graph_size != copy_out_counts.size())       // the graph is not empty (ie, num removed nodes != graph size)
        {
            for (u_int64_t i = 0; i < copy_out_counts.size(); i++)
            {
                // add node indices that have out-degrees > 0 (ie, are in the cycle)
                if (copy_out_counts[i] > 0)
...
Result find_deadlock(const std::vector<std::string> & edges)
{
    ...
    for (auto& edge : edges)
    {
        ...
        if (graph.topologicalSort(vec_proc_in_deadlock))    // check is there a cycle (true => has cycle)
        {
            ...
            for (auto & num : vec_proc_in_deadlock)
            {
                ...
                if (name[0] == processID)                   // check if its a proccess
                    vec_result_proc.emplace_back(name.substr(1));   // add it to `result.procs`, removing the proccessID at the front of the name
            }

            result.procs = vec_result_proc;

...
/// @param result a Result struct.
/// @param analyzeStruct a AnalyzeStruct struct.
/// @param dir_name the path of a directory, string.
/// @param emptyDirsOfChildren a vector<string>&, stores the top-level empty directories.
/// @return a boolean, of if a file was encountered in a directeory. Returns true if a file was found, false otherwise.
bool analyzeRecur(Results& result, AnalyzeStruct& analyzeStruct, string dir_name, vector<string>& topLevelVacentDirs)
{
    ...
    while(true)
    {
        ...
        if (is_dir(pathOfEntity))       // check if the `pathOfEntity` is a directory
        {
            ...
            bool subHasFile = analyzeRecur(result, analyzeStruct, pathOfEntity, emptyChildDirs);  // Recursively call `analyzeRecur(...)` on a sub-directory
...
void analyze(Results& result, int n)
{
    ...
    analyzeStruct.dir_root = ".";   // The `root` is the directory that main changed into.
    // analyzeRecur(result, mapOccurances, dir_name);
    // analyzeRecur(result, analyzeStruct, dir_name);
    analyzeRecur(result, analyzeStruct, dir_name, result.vacant_dirs);
